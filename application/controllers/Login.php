...
class Login extends CI_Controller {
    function __construct(){
        parent::__construct();
		$this->load->model('Md_login','Md_login');
    }
    ...
    function autentikasi(){
        ...
        $validasi_email = $this->Md_login->query_validasi_email($email);
        if($validasi_email->num_rows() > 0){
            $validate_ps=$this->Md_login->query_validasi_password($email,$password);
